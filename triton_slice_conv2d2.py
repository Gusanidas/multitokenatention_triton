        ...
        for out_w_idx in range(0, reduced_block_n):
            ...
            for kernel_q_idx in range(0, R):
                for kernel_k_idx in range(0, S):
                    ...
                    xx_h = out_h_idx + kernel_q_idx
                    xx_w = out_w_idx + kernel_k_idx
                    out = (
                        tl.sum(
                            tl.where(indicator == xx_h * BLOCK_SIZE_N + xx_w, x, 0.0)
                        )
                        * w
                    )
            acc = tl.where(indicator == out_h * BLOCK_SIZE_N + out_w, out, acc)
