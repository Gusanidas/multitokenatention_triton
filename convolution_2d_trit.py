    ...
    if short:
        return [
            triton.Config({'BLOCK_SIZE_M': bm, 'BLOCK_SIZE_N': bn, 'GROUP_SIZE_N': gm, 'SIZE_RS_BLOCKS': s}, num_stages=3,
                          num_warps=8) for bm in [64] for bn in [128] for gm in [2] for s in [1]
        ]
    else:
        return [
            triton.Config({'BLOCK_SIZE_M': bm, 'BLOCK_SIZE_N': bn, 'GROUP_SIZE_N': gm, 'SIZE_RS_BLOCKS': s}, num_stages=3,
...
def get_autotune_config_bwd_weight(short=SHORT_CONFIG):
    if short:
        ...
        return [
            triton.Config({'BLOCK_SIZE': bs, 'ID_BLOCK_SIZE': ib}, num_stages=3,
                          num_warps=8) for bs in [32,64,128,256] for ib in [1,3,16,32,64,128,256]
    ]
...
def conv_fwd_kernel(
    ...
    S: tl.constexpr,
    causal: tl.constexpr,
    BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr, GROUP_SIZE_N: tl.constexpr,
    ...
    kernel_id = tl.program_id(2)
    for kernel_b_id in range(0, R*S, SIZE_RS_BLOCKS):
        kernel_b_id = tl.multiple_of(kernel_b_id, SIZE_RS_BLOCKS)
        ii = kernel_b_id + kernel_id
        if ii < R*S:
            i = ii // S
            j = ii % S
    
            h_origin = h_start + i - pad_h
            w_origin = w_start + j - pad_w
            h = tl.arange(0, BLOCK_SIZE_M) + h_origin
            w = tl.arange(0, BLOCK_SIZE_N) + w_origin

            mask_h_left = h >= 0
            mask_h_right = h < H
            mask_w_left = w >= 0
            mask_w_right = w < W
            mask_h = mask_h_left & mask_h_right
            mask_w = mask_w_left & mask_w_right
            mask_input = mask_h[:, None] & mask_w[None, :]
            if causal:
                mask_causal = h[:,None] >= w[None,:]
                mask_input = mask_input & mask_causal

            x_block = input_ptr + batch_id * H * W * C + channel_id * H * W + h[:, None] * W + w[None, :]
            x = tl.load(x_block, mask=mask_input, other=0.0, cache_modifier=".ca")

            w_val = weight_ptr + channel_id * R * S + i * S + j
            w = tl.load(w_val)
            acc += x * w 
    ...
    o_block = output_ptr + batch_id * C * H * W + channel_id * H * W + o_h[:, None] * W + o_w[None, :]
    mask_o_block = mask_h_left[:, None] & mask_h_right[:, None] & mask_w_left[None, :] & mask_w_right[None, :]
    if causal:
        mask_causal = o_h[:,None] >= o_w[None,:]
        mask_o_block = mask_o_block & mask_causal

    if SIZE_RS_BLOCKS == 1:
        tl.store(o_block, acc, mask=mask_o_block)
    else:
        tl.atomic_add(o_block, acc, mask=mask_o_block)
...
def conv_bwd_input_kernel(
    ...
    for id_2 in range(0, R*S, SIZE_RS_BLOCKS):
        ...
        if ii < R*S:
            ...
            dout_block = dout_ptr + batch_id * C * P * Q + channel_id * P * Q + out_h[:, None] * Q + out_w[None, :]
    ...
    dx_block = dx_ptr + batch_id * H * W * C + channel_id * H * W + in_h[:, None] * W + in_w[None, :]
    if SIZE_RS_BLOCKS == 1:
    ...
    for start_id in range(0, total_ids, ID_BLOCK_SIZE):
        ...
        if id_2 < total_ids:
            ...
            if out_h_s >= out_w_s or not causal:
                ...
                x_idx = batch_id * H * W * C + channel_id * H * W + in_h[:, None] * W + in_w[None, :]
                x = tl.load(input_ptr + x_idx, mask=mask_in, other=0.0)

                dout_idx = batch_id * C * P * Q + channel_id * P * Q + out_h[:, None] * Q + out_w[None, :]
...
class Conv2dTritonFunction(Function):
    ...
    def forward(ctx, input, weight, causal):
        ...
        assert C == C_out, "This implementation assumes depthwise convolution (C_in == C_out)"
        ...
        grid = lambda META: (
            N * C,
            triton.cdiv(H, META['BLOCK_SIZE_M']) * triton.cdiv(W, META['BLOCK_SIZE_N']),
            META['SIZE_RS_BLOCKS']
        )
...
class PytorchConv(nn.Module):
    def __init__(self, channels, kernel_size, causal=False):
        ...
        self.kernel_size = kernel_size if isinstance(kernel_size, tuple) else (kernel_size, kernel_size)
        self.weight = nn.Parameter(torch.randn(channels, 1, self.kernel_size[0], self.kernel_size[1]))
        self.causal = causal
