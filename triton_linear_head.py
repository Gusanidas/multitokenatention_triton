...
def get_autotune_config(short=SHORT_CONFIG):
    if short:
        return [
            triton.Config(
                {"block_q": bq, "p_group_size": pg}, num_stages=ns, num_warps=nw
            )
            for bq in [64]
            for pg in [512]
            for ns in [3]
...
def get_autotune_config_bwd_input(short=SHORT_CONFIG):
    if short:
        return [
            triton.Config(
                {"block_q": bq, "p_group_size": pg}, num_stages=3, num_warps=8
            )
            for bq in [128]
            for pg in [256]
        ]
...
@triton.autotune(configs=get_autotune_config(), key=["N", "C", "L", "causal"])
@triton.jit
def linear_head_kernel_forward(
    ...
    linear_weight_block = (
        linear_weight_ptr + channel_off[:, None] + channel_off[None, :] * C
    )
    ...
    for p_id in range(0, P, p_group_size):
        ...
        if p_id + start_p_id < P and (
            not causal or p_id + start_p_id >= q_id * block_q
        ):
            p = start_p_id + p_id
...
@triton.autotune(configs=get_autotune_config_bwd_input(), key=["N", "C", "L", "causal"])
@triton.jit
def linear_head_kernel_backward_input(
    ...
    linear_weight_block = (
        linear_weight_ptr + tl.arange(0, L)[:, None] * C + tl.arange(0, L)[None, :]
    )
...
@triton.autotune(
    configs=get_autotune_config_bwd_weight(), key=["N", "C", "L", "causal"]
)
@triton.jit
...
class TritonLinearHeadFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, causal=True, output_value=0.0):
        ...
        assert weight.shape == (
            L,
            L,
        ), f"Weight must be square matrix, got {weight.shape}"
        ...
        grid = lambda META: (
            META["p_group_size"] * N * triton.cdiv(Q, META["block_q"]),
        )
...
class TritonLinearHead(nn.Module):
    ...
    def __init__(self, head_dim, causal=True, output_value=0.0):
        super().__init__()
        self.head_dim = head_dim
        self.weight = nn.Parameter(torch.randn(head_dim, head_dim) / (head_dim**0.5))
        self.causal = causal
    ...
    def forward(self, x):
        ...
        """
        return TritonLinearHeadFunction.apply(
            x, self.weight, self.causal, self.output_value
        )
